...
public abstract class Text
{
	...
	public static final Comparator<char[]> CHAR_ARRAY_COMP = (char[] o1, char[] o2) -> compare(o1, o2);
	...
	public static int compare(char[] cL, char[] cR)
	{
		int nCompare = 0;
		int nIndex = -1;
		int nLimit = Math.min(cL.length, cL.length);
		
		while (nCompare == 0 && ++nIndex < nLimit)
			nCompare = cL[nIndex] - cR[nIndex];
		
		if (nCompare == 0)
			nCompare = cL.length - cR.length;
   ...
   * @param nLength The maximum length of the truncated string.
	...
	public static byte[] decodeBase64Url(StringBuilder sEncoded)
	{
		int nRemainder = sEncoded.length() % 4;
		if (nRemainder == 2)
			sEncoded.append("..");
		if (nRemainder == 3)
			sEncoded.append(".");
		return UrlBase64.decode(sEncoded.toString());
	}
	
	
	public static SimpleDateFormat getISO8601Sdf()
	{
		return getISO8601Sdf(new SimpleTimeZone(0, ","));
	}
	
	
	public static SimpleDateFormat getISO8601Sdf(TimeZone oTz)
	{
		SimpleDateFormat oSdf = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSXXX");
		oSdf.setTimeZone(oTz);
		return oSdf;
	}
	
	
	public static char[] getCharArray(CharSequence sSeq)
	{
		int nLen = sSeq.length();
		char[] cRet = new char[nLen];
		for (int nIndex = 0; nIndex < nLen; nIndex++)
			cRet[nIndex] = sSeq.charAt(nIndex);
		
		return cRet;
	}
	
	
	public static void fillCharArray(CharSequence sSeq, char[] cArr)
	{
		for (int nIndex = 0; nIndex < cArr.length; nIndex++)
			cArr[nIndex] = sSeq.charAt(nIndex);
	}
}
